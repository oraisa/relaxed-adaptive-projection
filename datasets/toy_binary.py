...
if __name__ == "__main__":
    toy_binary = ToyBinary(False, "../data/", False, 1, 5)

    X = toy_binary.get_dataset()

    # generate random k-way attributes tuples at random
    kway_marginals = toy_binary.randomKway(num_kways=100, k=1)
    print(X.shape, len(kway_marginals), kway_marginals[0])

    # generate random N 3-way queries of indexes from kway_marginals
    kway_queries, num_queries = toy_binary.get_queries(kway_marginals)
